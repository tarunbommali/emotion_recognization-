...
import FrequencySpectrumChart from "./FrequencySpectrumChart"; // Import the 2D chart

// Helper Icon for upload (remains the same)
const UploadIcon = () => (
  <svg
...
// Helper function to get dynamic colors for emotion tags (remains the same)
...
function EEGProcessor() {
  ...
  // handleFileChange and handleSubmit remain largely the same
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.error) {
          ...
          setResponseInfo(null);
            ...
            if (!hasContent && !response.data.error) {
                setError("Processing complete, but no specific predictions or plot data were returned by the server.");
            }
        }
      } else {
      ...
      if (err.code === "ECONNABORTED") {
        setError("The request timed out. Server might be busy or the file is too large.");
      } else if (err.response && err.response.data && err.response.data.error) {
        setError(`Error from server: ${err.response.data.error}`);
      } else if (err.response && err.response.status) {
        setError(`Server returned an error: ${err.response.status} ${err.response.statusText || ''}`);
      } else if (err.request) {
        setError("Could not connect to the server. Ensure it's running.");
      } else {
      ...
      <div className="w-full max-w-4xl xl:max-w-7xl bg-white shadow-xl rounded-xl p-6 md:p-10 my-8 sm:my-12">
        <header className="mb-10 text-center">
          ...
          <p className="text-slate-600 text-lg sm:text-xl">
            Upload EEG CSV for multi-channel emotion prediction & spectrum analysis.
          </p>
        </header>
        ...
        {/* Results section */}
        {(!isLoading && !error && (predictions.length > 0 || epochPlotData.length > 0 || responseInfo)) && (
            <div className="mt-12 space-y-12">

            {/* Analysis Information section remains the same */}
            {responseInfo && (
                <section className="p-6 bg-slate-50 rounded-xl shadow">
                    ...
                    <div className="space-y-2 text-sm text-slate-600">
                        {responseInfo.processed_channels && Array.isArray(responseInfo.processed_channels) && (
                            <p><strong>Processed EEG Channels:</strong> {responseInfo.processed_channels.join(', ')}</p>
                        )}
                        {responseInfo.emotions_legend && responseInfo.emotions_legend !== "N/A (model has no classes_ attribute)" && (
                            <p><strong>Model Emotion Classes:</strong> {Array.isArray(responseInfo.emotions_legend) ? responseInfo.emotions_legend.join(', ') : responseInfo.emotions_legend}</p>
                        )}
                         {responseInfo.model_feature_columns && Array.isArray(responseInfo.model_feature_columns) && responseInfo.model_feature_columns.length > 0 && (
                            <p className="text-xs text-gray-500">({responseInfo.model_feature_columns.length} features used by model e.g., {responseInfo.model_feature_columns[0]})</p>
                        )}
                    </div>
                </section>
            )}

            {/* Predictions list remains the same */}
            ...
            {/* 3D Visualization section remains the same */}
